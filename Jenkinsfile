// ══════════════════════════════════════════════════════════════
// Jenkinsfile — Chatbot Legacy (Tầng 3)
...
pipeline {
    ...
    environment {
        PROJECT_DIR = '/root/Chatbot-CT-Group'
        COMPOSE_FILE = 'docker-compose.yml'
        APP_CONTAINER = 'legacy_chatbot_app'
        HEALTH_URL = 'http://localhost:7999/api/v1/health'
    }
        ...
        stage('Pull Latest Code') {
            steps {
                sh """
                    cd ${PROJECT_DIR}
                    git fetch origin
                    git reset --hard origin/refactor/mlops-3tier
                """
            }
        }
    ...
    post {
        success {
            echo '🎉 Chatbot Legacy deployed successfully!'
        }
        failure {
